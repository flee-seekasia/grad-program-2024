...
class FlakyTestRule : TestRule {
    ...
    private fun markTestAsFlaky(description: Description) {
        ...
        if (testInfo != null) {
            ...
            performFlakyMark(apiToken, orgSlug, suiteSlug, testInfo.id, testInfo.url, testName)
        } else {
     ...
     */
    private fun findTestInfo(apiToken: String, orgSlug: String, suiteSlug: String, testName: String): TestInfo? {
     ...
     * Reference: https://buildkite.com/docs/apis/rest-api/test-engine/tests#add-or-remove-labels-from-a-test
     * 
     * The labels API requires team UUID and suite UUID. This implementation:
     * 1. First tries the analytics endpoint pattern (simpler, may work)
     * 2. Falls back to the full labels endpoint if team/suite UUIDs are available
     */
    private fun performFlakyMark(apiToken: String, orgSlug: String, suiteSlug: String, testId: String, testUrl: String, testName: String) {
        // Try analytics endpoint pattern first (simpler, may be supported)
        val analyticsUrl = "https://api.buildkite.com/v2/analytics/organizations/$orgSlug/suites/$suiteSlug/tests/$testId/labels"
        ...
        // Fallback: Try full labels endpoint with team/suite UUIDs
        val teamUuid = System.getenv("BUILDKITE_TEAM_UUID")
        val suiteUuid = System.getenv("BUILDKITE_SUITE_UUID")
        
        if (teamUuid != null && suiteUuid != null) {
            val labelsUrl = "https://api.buildkite.com/v2/organizations/$orgSlug/teams/$teamUuid/suites/$suiteUuid/tests/$testId/labels"
            ...
            try {
                val response = client.newCall(labelsRequest).execute()
                if (response.isSuccessful) {
                    println("✅ Successfully added 'flaky' label via labels endpoint: $testName")
                    return
                } else {
                    val errorBody = response.body?.string() ?: "Unknown error"
                    println("⚠️  Failed to add flaky label via labels endpoint: ${response.code} $errorBody")
                }
            } catch (e: Exception) {
                println("⚠️  Error adding flaky label via labels endpoint: ${e.message}")
            }
        } else {
            println("⚠️  BUILDKITE_TEAM_UUID and BUILDKITE_SUITE_UUID not set.")
            println("   Set these environment variables to use the full labels API endpoint.")
        }
